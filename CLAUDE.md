...
**Model selection** is per-workload via `lib/ai/model-config.ts` (`modelFor(workload)`), NOT a single global. Defaults: synthesis→`claude-fable-5`, board→`claude-opus-4-8`, chat→`claude-sonnet-4-6`, util→`claude-haiku-4-5`. Override per workload with `ANTHROPIC_MODEL_{SYNTHESIS,BOARD,CHAT,UTIL}`, or set `ANTHROPIC_MODEL` as a global kill-switch (forces every workload onto one model — use for rollback or before Fable 5 / Opus 4.8 access is enabled). Frontier models (Fable 5, Opus 4.8/4.7) 400 on `temperature` — use `samplingFor(model, t)` to build the sampling slice, never hardcode `temperature` in a `messages.create()` call. Reasoning effort: synthesis/board send `output_config.effort: "high"` via `effortConfigFor(model, workload)` (model-gated — stripped on models that don't support effort); tune with `ANTHROPIC_EFFORT_{SYNTHESIS,BOARD}` (`low|medium|high|max`).
