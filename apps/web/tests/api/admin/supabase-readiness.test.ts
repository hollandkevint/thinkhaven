...
vi.mock('@/lib/beta/supabase-readiness', () => ({
  checkRailwayReadiness: vi.fn(),
}));

describe('admin Railway readiness API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    vi.mocked(checkRailwayReadiness).mockResolvedValue({
      status: 'warn',
  ...
  it('blocks non-admin callers before running checks', async () => {
    ...
    expect(checkRailwayReadiness).not.toHaveBeenCalled();
  });
});
