...
- **Tech Stack**: Next.js 15.5, React 19, TypeScript, Supabase, Stripe, Anthropic Codex
...
- Unit test debt is tracked, not hidden: a subset of unit test files have known pre-existing failures (documented in `docs/solutions/`); the authoritative ship gates are the E2E suite (green in CI) and the production checklist (`npm run test:prod`). mary-persona: 67/67 pass.
...
8. **Tool results** - Use `ToolExecutor.formatResultsForClaude()` for Codex's expected `tool_result` format
...
14. **Codex Review CI** - `anthropics/Codex-action@v1` has known SDK crash (issue #911). `continue-on-error: true` is set. Job may show failed but workflow passes.
