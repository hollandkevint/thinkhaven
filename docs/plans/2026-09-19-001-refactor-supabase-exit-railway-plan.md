...
3. Streaming chat, message persistence, counters, canvas, artifacts, and AI tools. The authenticated stream boundary, browser messages, canvas merges, document generation, phase completion, insights, and model-triggered session tools now use actor-scoped PostgreSQL. Legacy context/export helpers remain.
4. Feedback, legacy conversations, monitoring, public sharing, and exports. Feedback and public artifact paths now use parameterized PostgreSQL. Assessment retains its existing local fallback because its source table is absent. Legacy conversations and export remain.
...
- Chat stream ownership, message limits, titles, persona state, canvas refresh, idempotent browser message appends, feedback, beta status/readiness, canvas/document tools, and session primitives now use authenticated PostgreSQL paths. The remaining live references are context/export, guest migration, a legacy conversation stack, obsolete auth callback/helpers, and environment validation.
