...
2. Search codebase for any imports of `@supabase/auth-helpers-nextjs` and update to use `@supabase/ssr`:
   - Pattern to find: `from '@supabase/auth-helpers-nextjs'`
   - Replace with appropriate `@supabase/ssr` imports
3. Run `npm install` to update lockfile
4. Fix any TypeScript errors from removed imports

Key imports to replace:
- `createMiddlewareClient` -> use `createServerClient` from `@supabase/ssr`
- `createClientComponentClient` -> keep using `createBrowserClient` from `@supabase/ssr`
- `createServerComponentClient` -> keep using `createServerClient` from `@supabase/ssr`

Do NOT change files that already import from `@supabase/ssr` - they're already correct.
  </action>
  <verify>
