...
<task type="auto">
  <name>Task 2: Audit other /dashboard references in auth flows</name>
  <files>apps/web/lib/supabase/middleware.ts, apps/web/app/login/*, apps/web/app/signup/*</files>
  <action>
Search for other `/dashboard` references in auth-related files and update to `/app`:

1. Search: `grep -r "/dashboard" apps/web/app/login/ apps/web/app/signup/ apps/web/lib/supabase/ apps/web/lib/auth/`
2. For each match, evaluate if it should be `/app` instead
3. Update redirect destinations from `/dashboard` to `/app`

Common patterns to update:
- `url.pathname = '/dashboard'` -> `url.pathname = '/app'`
- `redirect('/dashboard')` -> `redirect('/app')`
- `router.push('/dashboard')` -> `router.push('/app')`

Skip:
- Test files
- Comments mentioning the old path
- URLs in error messages (leave as-is for clarity)
  </action>
  <verify>
- `grep -r "pathname.*dashboard" apps/web/app/ apps/web/lib/` returns only test files or comments
- All auth success flows redirect to /app
- `npm run build` succeeds
  </verify>
  <done>
- All auth-related redirects point to /app
