...
must_haves:
  truths:
    - "All API routes use getUser() not getSession() for auth"
    - "All lib files use getUser() not getSession() for auth"
    - "Security vulnerability from getSession() is eliminated"
  artifacts:
...
<task type="auto">
  <name>Task 1: Audit all getSession() usages in production code</name>
  <files>N/A (audit only)</files>
  <action>
Run comprehensive search to find all getSession() usages:

```bash
grep -rn "getSession()" apps/web/app/api/ apps/web/lib/ --include="*.ts" --include="*.tsx" | grep -v "node_modules" | grep -v ".test." | grep -v "__tests__"
```

Create a list of files that need updating. For each file, note:
- File path
- Line number
- Current usage pattern
- Whether it's checking for user authentication

Exclude test files - they can keep getSession() for mocking purposes.
  </action>
  <verify>
- Audit list created
- Each file categorized as: needs-update, already-correct, or test-file
  </verify>
  <done>
- Complete list of files using getSession()
- Clear understanding of which files need updating
  </done>
</task>

<task type="auto">
  <name>Task 2: Update API routes to use getUser()</name>
  <files>apps/web/app/api/*/route.ts, apps/web/app/api/**/route.ts</files>
  <action>
For each API route using getSession(), update to use getUser():
...
<task type="auto">
  <name>Task 3: Update lib files to use getUser()</name>
